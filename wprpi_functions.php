	...
	function wprpi_related_content( $content ) {
		...
		if( is_singular() && is_main_query() && !$shortcode_exist ) {
			...
			if( $count_para > $random_show ){
				...
				if( $related ){
					$content .= '<div class="wp_random_inside">'.$icon_val.'<a href="'.esc_url(get_permalink( $related[0] )).'">'.get_the_title( $related[0] ).'</a> ('.get_the_date( 'j F Y', $post->ID ).')</div>';
				}
				
				$content .= implode( '</p>', $paragraph );
				if( count($related) > 1 ){
					$content .= '<div class="wp_random_inside">'.$icon_val.'<a href="'.esc_url(get_permalink( $related[1] )).'">'.get_the_title( $related[1] ).'</a> ('.get_the_date( 'j F Y', $post->ID ).')</div>';
				}
				...
				if( $related ){
					$content .= '<div class="wp_random_inside">'.$icon_val.'<a href="'.esc_url(get_permalink( $related[0] )).'">'.get_the_title( $related[0] ).'</a> ('.get_the_date( 'j F Y', $post->ID ).')</div><p>';
				}
				
				$content .= implode( ' ', $content_main );
				
				if( count($related) > 1 ){
					$content .= '<div class="wp_random_inside">'.$icon_val.'<a href="'.esc_url(get_permalink( $related[1] )).'">'.get_the_title( $related[1] ).'</a> ('.get_the_date( 'j F Y', $post->ID ).')</div><p>';
				}
			}
		...
		foreach ($related as $post) {
			...
			if( $wprpi_value['thumb_excerpt'] ){
				...
				$wprpi_val .= '<h4 class="wprpi-post-title">
									'.$icon_val.'<a href="'.esc_url(get_permalink( $post->ID )).'">'.get_the_title( $post->ID ).' ('.get_the_date( 'j F Y', $post->ID ).')</a>
							   </h4>';
				$wprpi_val .= $thumbnail;
				...
				$wprpi_val .= $icon_val;
				$wprpi_val .= '<a href="'.esc_url(get_permalink( $post->ID )).'">'.get_the_title( $post->ID ).' ('.get_the_date( 'j F Y', $post->ID ).')</a>';
			}
			$wprpi_val .= '</div>';
		}
